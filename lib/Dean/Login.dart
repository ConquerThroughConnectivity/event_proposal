...
class DeanLogin extends StatefulWidget {
  
  @override
  _DeanLoginState createState() => _DeanLoginState();
}

class _DeanLoginState extends State<DeanLogin> {
                  ...
                  child: Column(
                    children: <Widget>[
                      ...
                      onPressed: () async {
                            ...
                            FirebaseDatabase.instance.reference().child("User").child("Approver").orderByChild("org_type").equalTo("Dean's Office")
                            .once().then((DataSnapshot snapshot) async{
                              Map<dynamic, dynamic> values =await snapshot.value;
                              if(values==null){
                                setState(() {
                                  isload =false;
                                  popupInvalid("No Dean's Office Registered");
                                });
                              }else{
                                values.forEach((key,values){
                                  if(values["id"]==schoolID.text && values["password"]==password.text){
                                    ...
                                    Navigator.pushReplacement(context,MaterialPageRoute(builder: (context) =>DeanHome(id: values["id"],), fullscreenDialog: true));
                                  });
                                  }else{
                                  setState(() {
                                    isload =false;
